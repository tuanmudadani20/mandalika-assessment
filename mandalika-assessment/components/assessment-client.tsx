        ...
        {stage !== 'intro' ? (
          <section className="mt-2.5 grid h-[calc(100vh-82px)] gap-2 xl:grid-cols-[minmax(0,1fr)_248px]">
            <section className="order-1 flex h-full min-h-0 flex-col gap-3">
              ...
              {stage !== 'submitting' ? <QuestionStage stage={stage} meta={meta} tetradIndex={tetradIndex} sjtIndex={sjtIndex} essayIndex={essayIndex} tetradAnswers={tetradAnswers} sjtAnswers={sjtAnswers} essayAnswers={essayAnswers} setEssayAnswers={setEssayAnswers} updateChoice={updateChoice} /> : null}
            </section>
            <aside className="order-2 space-y-2.5 xl:order-2 xl:sticky xl:top-24 xl:self-start">
              <div className="surface-card p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="rounded-field border border-border bg-panel px-3 py-1 text-[11px] text-text">
                    {activeIndex + 1}/{activeNumbers}
                  </span>
                </div>
                <p className="mt-1 text-[12px] leading-5 text-muted">Estimasi total 75 menit</p>
                ...
                <div className="mt-3 flex gap-2">
                  <button type="button" className="btn-secondary w-1/2 px-3 py-2 text-[12px]" disabled={stage === 'submitting'} onClick={goPrev}>
                    Sebelumnya
                  </button>
                  <button type="button" className="btn-primary w-1/2 px-3 py-2 text-[12px]" disabled={stage === 'submitting'} onClick={goNext}>
                    {nextLabel}
                  </button>
                </div>
              </div>
...
function QuestionStage({
  ...
  essayAnswers: string[]
  setEssayAnswers: React.Dispatch<React.SetStateAction<string[]>>

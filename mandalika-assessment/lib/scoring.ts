...
export function computeScores(
  ...
  const rawCategory = getCategoryFromScore(weightedScoreExact)
  const downgradeReasons: string[] = []
  let cap: PlayerCategory = 'A Player'

  if (criticalCount >= 1) {
    cap = lowerCategory(cap, 'B Player')
    downgradeReasons.push('Critical failure pada Gate 1')
  }
  if (failedCount >= 2) {
    cap = lowerCategory(cap, 'B Player')
    downgradeReasons.push('Dua atau lebih dimensi Gate 1 tidak lolos')
  }
  if (failedCount === 1) {
    cap = lowerCategory(cap, 'B Solid Player')
    downgradeReasons.push('Satu dimensi Gate 1 tidak lolos')
  }
  if (!gate2Passed) {
    cap = lowerCategory(cap, 'B Player')
    downgradeReasons.push('Rata-rata Layer 2 di bawah threshold')
  }

  const finalCategory = lowerCategory(rawCategory, cap)
  const wasDowngraded = finalCategory !== rawCategory
  const finalScoreExact = wasDowngraded
    ? Math.min(weightedScoreExact, categoryCaps[finalCategory])
    : weightedScoreExact
  ...
  return {
    ...
    finalCategory,
    wasDowngraded,
    downgradeReason: wasDowngraded ? downgradeReasons.join('; ') : undefined,
    gate1: {
      passed: gate1Passed,
      failedCount,
      criticalCount,
